...
const StatCard = ({ label, value, bgColor }: { label: string; value: number; bgColor: string }) => (
  <div className={cn("rounded-lg p-4 flex items-center justify-between", bgColor)}>
    <span className="font-bold text-lg uppercase tracking-wider text-background">
      {label}
    </span>
    <span className="text-4xl font-bold text-background">{value}</span>
  </div>
);
